...
function BlockRowConst(props) {
    ...
    return (
        <div id={row._id} className="block" style={{backgroundColor: block.color}}>
            {block.complex === 0 ?          
                <DescriptionConst block={block} row={row} list_num={0}></DescriptionConst> 
                :
                Array(block.complex).fill(null).map((list_value, list_num) => (
                    <div key={list_num}>
                        <div className='description'> <DescriptionConst block={block} row={row} list_num={list_num}></DescriptionConst> </div>
                        {
